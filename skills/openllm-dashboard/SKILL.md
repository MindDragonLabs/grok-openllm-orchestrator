...
description: Use OpenLLM MCP tools for dashboard-like account work (models, usage, keys/devices, providers, config). Use when the user asks to inspect or manage their OpenLLM account without relying on the web UI.
---
...
You are still the orchestrator. OpenLLM exposes a native gateway API as MCP tools (one tool per OpenAPI operation from `/api/swagger`). **Use the live tool list.** The operation ids below are from the public CLI-embedded spec so you can *recognize* surfaces — they are not a promise of exact MCP function names.
...
- Never paste provider API keys, vault recovery phrases, or raw `sk-llm` secrets into the repo or a PR.
