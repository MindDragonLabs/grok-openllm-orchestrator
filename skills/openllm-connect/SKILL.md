...
Upstream: [OpenLLM CLI](https://github.com/openllmsh/cli). Config keys documented there: `OPENLLM_API_KEY` and `OPENLLM_CLOUD_ORIGIN` (environment **or** `~/.openllm/.env` from daemon pairing).
...
Import companion skills from this repo (or copy skill prose into Grok Bot skills). Keep GitHub, Vercel, and other connectors attached — OpenLLM is not those products. Full playbook: **grok-bot-orchestrator** and [docs/grok-bot.md](../../docs/grok-bot.md).
