...
Copy-pasteable path for **Grok Bot owners**. Grok Bot is the orchestrator; OpenLLM is the model fabric. Full architecture: [architecture.md](./architecture.md).

This plugin teaches Cursor and Grok Bot the same split. Grok Bot still needs its **own** OpenLLM connector: a shared bot template cannot pack custom MCP for the next owner, and Cursor `mcp.json` is not copied over.
...
From [https://github.com/MindDragonLabs/grok-openllm-orchestrator](https://github.com/MindDragonLabs/grok-openllm-orchestrator):
...
| `skills/grok-bot-orchestrator` | This playbook |

Install this repo as a Cursor/Grok plugin when available, **or** copy each `SKILL.md` body into Grok Bot skills.

Also keep the rule `rules/openllm-orchestrator.mdc` in mind: discover live MCP tool names; never invent them.
...
| | Cursor IDE | Grok Bot |
| --- | --- | --- |
| This plugin's `mcp.json` | Loaded after install + variables | Not automatic — add custom MCP |
...
| Skills/rules/commands | From the plugin | Import plugin or copy skills |
